...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Perform Claim Function
        const gasPrice = await web3.eth.getGasPrice();
        const nonce = await web3.eth.getTransactionCount(address);
        const tx = dripContract.methods.claim();
        ...
        if (receipt && receipt.transactionHash) {
            ...
            if (transactionMined) {
                ...
                // response
                res.status(200).json({message: 'Claim transaction mined'});
            }
        }
    } catch (error) {
        console.log(error);
        res.status(503).json({ message: 'Claim Action Failed' });
    }
}
